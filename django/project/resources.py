...
class SolutionsResource(resources.ModelResource):
    ...
    people_reached = Field(
        column_name=_("People reached [if empty -> sum of countries]")
    )
    list_of_countries = Field(
        column_name=_("Countries [Country - Region - People reached]")
    )
    ...
    def dehydrate_list_of_countries(self, solution):  # pragma: no cover
        ...
        countries_with_people_reached = [
            "{} - {} - {}".format(
                cwd.country, CountryOffice.REGIONS[cwd.region][1], cwd.people_reached
            )
            for cwd in countries_with_data
        ]
...
class ProjectResource(resources.ModelResource):  # pragma: no cover
    ...
    leading_sector = Field(column_name=_("Leading Sector"))
    supporting_sector = Field(column_name=_("Supporting Sector"))
    ...
    def export_queryset(
        self, queryset, using_transactions=True, use_bulk=True, **kwargs
    ):
        if use_bulk:
            return self.export_bulk(queryset, **kwargs)
        else:
            return super().export_queryset(
                queryset, using_transactions=using_transactions, **kwargs
            )
    ...
    def dehydrate_contact(self, project):
        ...
        return (
            f'{data.get("contact_name", "None")} <{data.get("contact_email", "None")}>'
        )
    ...
    def dehydrate_unicef_office(self, project):
        try:
            return CountryOffice.objects.get(
                id=self.get_data_member(project).get("country_office")
            ).name
    ...
    def dehydrate_country(self, project):
        try:
            return Country.objects.get(
                id=self.get_data_member(project).get("country")
            ).name
    ...
    def dehydrate_supporting_sector(self, project):
        sector = self.get_data_member(project).get("unicef_supporting_sector")
        qs = UNICEFSupportingSector.objects.filter(id__in=supporting_sector)
        return ", ".join(res.name for res in qs) if qs.count() else None

    def dehydrate_goal_area(self, project):
        try:
            return UNICEFGoal.objects.get(
                id=self.get_data_member(project).get("goal_area")
            ).name
    ...
    def dehydrate_result_area(self, project):
        try:
            return UNICEFResultArea.objects.get(
                id=self.get_data_member(project).get("result_area")
            ).name
    ...
    def dehydrate_innovations(self, project):
        ...
        if ids:
            names = InnovationWay.objects.filter(
                id__in=self.get_data_member(project).get("innovation_ways")
    ...
    def dehydrate_cpd(self, project):
        ...
        try:
            return ", ".join(
                CPD.objects.filter(id__in=cpd).values_list("name", flat=True)
            )
    ...
    def dehydrate_partners(self, project):
        ...
        for partner in self.get_data_member(project)["partners"]:
            p_type = (
                p_type_lookup[partner["partner_type"]]
                if "partner_type" in partner
                else "n/a"
            )
            p_string = (
                f'{p_type}: {partner.get("partner_name")} ({partner.get("partner_contact")} - '
